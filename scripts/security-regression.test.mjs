...
test("cacheable gallery and feed APIs use ETags before expensive reads", () => {
  ...
  const indexedTagDetail = sourceBlock(workerRoutes, "async function indexedTagDetailForUser", "async function tagDetailForUser");
  assert.match(indexedTagDetail, /serializeGalleryWorkListItem/);
  assert.doesNotMatch(indexedTagDetail, /serializeWork\(c\.env, user, work\)/);
  assert.match(indexedTagDetail, /current_work_collaborator/);
  assert.match(indexedTagDetail, /feedback_request_dismissals/);
  assert.match(indexedTagDetail, /publicUsersWithTags\(c\.env\.DB, memberRows\.results\)/);
  assert.match(indexedTagDetail, /commentsWithVisibilityAndReactions\(c, user, commentRows\.results/);
  const tagMemberBatch = sourceBlock(workerRoutes, "async function publicUsersWithTags", "function targetKey");
  assert.match(tagMemberBatch, /WHERE user_id IN/);
  const tagCommentVisibility = sourceBlock(workerRoutes, "async function visibleTargetKeysForUser", "async function commentsWithVisibilityAndReactions");
  assert.match(tagCommentVisibility, /visibleGalleryIds\(c\.env\.DB, user, galleryIds\)/);
  assert.match(tagCommentVisibility, /visibleWorkIds\(c\.env\.DB, user, workIds\)/);
  const tagCommentBatch = sourceBlock(workerRoutes, "async function commentsWithVisibilityAndReactions", "async function legacyTagDetailForUser");
  assert.match(tagCommentBatch, /visibleTargetKeysForUser\(c, user, rows\)/);
  assert.match(tagCommentBatch, /FROM reactions[\s\S]*target_id IN/);
  assert.doesNotMatch(tagCommentBatch, /reactionSummary/);
...
test("protected media opens a swipeable gallery lightbox", () => {
  ...
  assert.match(workViewSource, /class="version-grid"/);
  assert.match(workViewSource, /class="version-thumb"/);
  assert.match(stylesSource, /\.version-grid[\s\S]*grid-template-columns: repeat\(auto-fill, minmax\(180px, 240px\)\)/);
