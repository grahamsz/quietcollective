...
app.get("/api/activity", async (c) => {
  const user = currentUser(c);
  const rows = await c.env.DB.prepare("SELECT * FROM domain_events ORDER BY created_at DESC LIMIT 100").all<ActivityRow>();
  const events = [];
  for (const event of rows.results) {
    if (await eventVisibleTo(c.env.DB, user, event)) events.push(await activityEntry(c.env, user, event));
  }
